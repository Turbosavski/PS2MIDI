...
// Every call to read() returns a single byte for each keystroke.  
// These configure what byte will be returned for each "special" key. 
// To ignore a key, use zero.
...
extern const PROGMEM PS2Keymap_t PS2Keymap_MIDI;
...
class PS2Keyboard {
  public:
    ...
    static bool availablerel(); // check if keys are available, including key releases
    /**
     * Returns the char last read from the keyboard.
     * If there is no char available, -1 is returned.
     */
    static int read();  
    static int readrel(bool &iskeyrelease); // check if keys are available, including key releases
  ...
  private :
    static int readrel_internal(bool &iskeyrelease, bool detectkeyreleases); // check if keys are available, including key releases
};
